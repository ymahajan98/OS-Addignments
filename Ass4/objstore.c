...
static pthread_mutex_t lock_inode;
static pthread_mutex_t lock_data;
...
static int write_cached(struct objfs_state *objfs,char *buf,int block){
    ...
    int offing =block%CACHE_BLOCKS;
    offing=offing<<12;
    char *cache_write=objfs->cache+offing;
...
static int find_cache(struct objfs_state *objfs,int block){
    int cache_index=block/(CACHE_BLOCKS);
    int cache_off=block%(CACHE_BLOCKS);
    if(cache_array[cache_off]==cache_index){
        cache_array[cache_off]=-1;
        dirty_cache[cache_off]=0;
    }
    return 0;
}
                ...
                if(!strcmp(cnt->key,key)){
                    ...
                    for(int l=0;l<4;l++){
                        if(cnt->indirect[l]!=0){
                           ...
                           #ifdef CACHE
                               read_cached(objfs,(char*)tem,cnt->indirect[l]-WRITE_OFFSET);
                           #else
                           ...
                           for(int al=0;al<1024;al++){
                                if(tem[al]){
                                    ...
                                    #ifdef CACHE
                                        find_cache(objfs,del1);
                                    #endif
                                }
                           }
                           bit_map_data[k]^=(1<<mo);
                           cnt->indirect[l]=0;
                           free_4k(tem,BLOCK_SIZE);
                           #ifdef CACHE
                             find_cache(objfs,del);
                           #endif
                        }
    ...
    for(int i=0;i<(DATA_BLOCKS*BLOCK_SIZE)/4;i++){
        ...
        for(int j=0;j<32;j++){
            if((bit_map_data[i]&val)==0){
                ...
                #ifdef CACHE
                    write_cached(objfs,(char *)abc,i*32+j);
                #else
                ...
                if(to_write->indirect[alpha]==0){
                    ...
                    #endif
                    to_write->size+=size;
                    free_4k(indirect_array,BLOCK_SIZE);
                    free_4k(abc,BLOCK_SIZE);
                    return size;
                }
                else{
...
long objstore_read(int objid, char *buf, int size, struct objfs_state *objfs,off_t offset){
    ...
    for(int i=0;(i<4)&&(total_blocks>0);i++){
        ...
        #ifdef CACHE
             read_cached(objfs,(char *)((char *)read_array+iterator*BLOCK_SIZE),to_read->direct[i]-WRITE_OFFSET);
        #else
    ...
    for(int j=0;j<4&&(total_blocks>0);j++){
        int *temp;
...
/*
        ...
        j   
*/
void init_free(struct objfs_state *objfs){
    for(int i=0;i<31;i++){
...
int objstore_destroy(struct objfs_state *objfs){
    ...
    free_4k(do_it,BLOCK_SIZE);
    for(int off=0;off<CACHE_BLOCKS;off++){
        if(dirty_cache[off]==1){
            if(cache_array[off]>=0){
               int block_number=cache_array[off]*CACHE_BLOCKS+off;
               char *cache_write=objfs->cache+(off<<12);
               write_block(objfs,WRITE_OFFSET+block_number,cache_write);
            }
        }
    }
    for(int i=0;i<31;i++){
    ...
    free_4k(cache_array,CACHE_BLOCKS*4);
    free_4k(dirty_cache,CACHE_BLOCKS*4);
    return 0;
}
