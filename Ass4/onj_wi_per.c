               ...
               // dprintf("setting %d\n",i*32+j);
                if(offie<4){
                    if(to_write->direct[offie]==0){
                        ...
                        to_write->size=to_write->size+BLOCK_SIZE;
                    }
                    else{
...
int objstore_init(struct objfs_state *objfs){
    ...
    for(int i=0;i<31;i++){
        read_block(objfs,i,(char *)((char *)bit_map_inode+(i+OBJECT_BLOCK)*BLOCK_SIZE));
    }
    for(int i=0;i<256;i++){
        read_block(objfs,i,(char *)((char *)bit_map_data+(i+OBJECT_BLOCK+31)*BLOCK_SIZE));
    }
